...
import { Message, RequestRecord, ContactMap } from '../../sdk/models/types.ts';

export const ChatView: React.FC = () => {
    const { me, chatConIdPublico, setChatConIdPublico, mostrarChatMobile, setMostrarChatMobile } = useStore();
    ...
    const contactList = (
        <div className="contact-list-pane" style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '10px', overflowY: 'auto' }}>
            <Button onClick={() => useStore.setState({ showModalAdd: true })}>+ Añadir Nodo</Button>
            {requests.length > 0 && (
